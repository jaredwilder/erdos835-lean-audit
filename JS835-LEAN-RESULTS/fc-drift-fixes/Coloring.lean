...
-- VENDOR NOTE (Oracle, 2026-07-28): `le_chromaticNumber_iff_colorable` and
-- `le_chromaticNumber_iff_coloring` were UPSTREAMED into Mathlib between v4.27 (this library's
-- pin) and v4.29.1 (ours), so re-declaring them here is a hard `already been declared` error.
-- Removed, NOT weakened: both names still resolve, to Mathlib's copies, whose statements were
-- checked against these by `#check` and are identical.
...
-- VENDOR NOTE (Oracle, 2026-07-28): `Coloring.injective_comp_of_pairwise_adj`,
-- `Colorable.card_le_of_pairwise_adj` and `le_chromaticNumber_of_pairwise_adj` were likewise
-- upstreamed into Mathlib by v4.29.1. Removed, NOT weakened -- `#check` confirmed Mathlib's
-- statements are identical, and every later use in this file resolves to them.
...
lemma card_div_indepNum_le_chromaticNumber : ⌈(Nat.card V / α(G) : ℚ≥0)⌉₊ ≤ G.chromaticNumber := by
  ...
  rw [Finset.mem_coe, Finset.mem_bipartiteBelow] at hx hy
...
theorem colorable_iff_induce_eq_bot (G : SimpleGraph V) (n : ℕ) :
  ...
  -- VENDOR NOTE (Oracle, 2026-07-28): was `by aesop`, which no longer closes this goal under
  -- Mathlib v4.29.1. The STATEMENT is untouched; only the proof term is repaired. Two vertices in
  -- the same colour class share a colour, so a proper colouring forbids an edge between them.
  refine ⟨fun ⟨a, h⟩ ↦ ⟨a, fun i ↦ ?_⟩, fun ⟨w, h⟩ ↦ ⟨w, @fun a b h_adj ↦ ?_⟩⟩
  · ext ⟨x, hx⟩ ⟨y, hy⟩
    simp only [comap_adj, Function.Embedding.coe_subtype, bot_adj, iff_false]
    intro hadj
    exact h hadj (hx.trans hy.symm)
  specialize h (w a)
  contrapose h
  intro hG
